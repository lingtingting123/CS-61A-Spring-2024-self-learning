class Transaction:
    ...
    def report(self):
        ...
        if self.changed():
            "*** YOUR CODE HERE ***"
            msg = ('increased ' if self.after > self.before else 'decreased ') + str(self.before) + '->' + str(self.after)
...
class Account:
    ...
    def deposit(self, amount):
        ...
        """
        self.num_transactions += 1
        self.transactions += [Transaction(self.num_transactions,self.balance,self.balance + amount)]
    ...
    def withdraw(self, amount):
        ...
        if amount > self.balance:
            self.transactions += [Transaction(self.num_transactions,self.balance,self.balance)]
...
class ChangeMachine:
    ...
    def change(self, coin):
        ...
        if coin in self.coins:
            ...
            self.coins[coin] = 1
        remove_list = make_change(coin,self.coins)
        for coin_remove in remove_list:
            if self.coins[coin_remove] == 1:
                self.coins.pop(coin_remove)
            else:
                self.coins[coin_remove] -= 1
        return remove_list
