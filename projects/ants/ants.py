    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            if place.ant.is_container and place.ant.can_contain(self):
                ...
                place.ant = self
            assert place.ant is None or can_add, 'Too many ants in {0}'.format(place)
            # END Problem 8b
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        k = 0
        while not goal_place.is_hive and (not goal_place.bees or k < self.lower_bound) and k < self.upper_bound:
            goal_place = goal_place.entrance
            k += 1
        if goal_place.is_hive:
            return None
        goal_bee = random_bee(goal_place.bees)
        return goal_bee
        # END Problem 3 and 4
...
class QueenAnt(ThrowerAnt):
    ...
    def action(self, gamestate):
        ...
        while behind_place:
            ...
            if ant_behind:
                if ant_behind.is_container:
                    if ant_behind.ant_contained and not ant_behind.ant_contained.is_doubled:
                        ant_behind.ant_contained.double()
                if not ant_behind.is_doubled:
                    ant_behind.double()
...
class LaserAnt(ThrowerAnt):
    ...
    def insects_in_front(self):
        ...
        place_infront = self.place.entrance
        distance = 1
        while not place_infront.is_hive:
            ant = place_infront.ant
            if ant:
                if ant.is_container and ant.ant_contained:
                    insects[ant.ant_contained] = distance
                insects[ant] = distance
            bees = place_infront.bees
            if bees:
                for bee in bees:
                    insects[bee] = distance
            distance += 1
            place_infront = place_infront.entrance
        return insects
        # END Problem Optional 2

    def calculate_damage(self, distance):
        # BEGIN Problem Optional 2
        damage = LaserAnt.damage - distance * 0.25
